# C++ Plan Simplicity Audit

## Context

The abandoned monolithic C++ review preparation was redirected into seven independently executable audit Plans. This seven-Plan suite is a one-time migration audit because the review-gate skills and agents were added recently and this pre-existing codebase has never passed through them; execution is a quick scan for clear violations of the new review rules. This Plan owns only the plan-simplicity lens over the current first-party C++ corpus. At execution, freeze one clean Git SHA and use the authoritative classifier rules from `.agents/scripts/Get-SessionChangeInventory.ps1` against `git ls-tree -r <SHA>` to enumerate first-party C++ targets under `Common/`, `DataPacker/`, `Engine/`, `Projects/`, and `Tools/`, excluding `ThirdParty/` and generated paths. Current evidence is 529 C++ targets and exactly four dual-language headers. The audit is findings-only and does not change source or add capability.
...
2. Start one fresh Luna `researcher` per shard, with fork none and no children. Each researcher owns a unique ignored report at `Temp/CppPlanSimplicityAudit/<sha>/<shard>.md`, covers every assigned file, and may read callers, consumers, producers, and mirrors outside the shard while keeping the candidate anchor/root in the owned shard. Reports assign deterministic candidate IDs (recommended form `CPS/<shard>/<###>`, with the frozen SHA in report metadata) and record severity `LOW`/`MEDIUM`/`HIGH`/`CRITICAL`, confidence, occurrence `observed`/`credible_exposure`/`hypothetical`, path:line and symbol, controlling directive or review concept, claim, evidence, reachability, refutation, impact, next step, and any external claim. Hypothetical candidates are at most MEDIUM; CRITICAL is catastrophic/core; HIGH is major and credible or observed; MEDIUM is localized; LOW is weak or cosmetic.
...
4. For each occurrence, classify it as observed, credible exposure, or hypothetical; identify who is harmed and when; compare the proposed machinery's implementation, maintenance, runtime, and integration cost with a simpler current mechanism, deleting the requirement, fixing the origin, or accepting/deferring the risk. Check the actual consumer and observable benefit, pass-through signals, representation fit, and whether a proposed option, hook, registry, or format has a current consumer. For a guard or workaround, distinguish root-cause removal from a bandaid and weigh the ASSERT-or-defer floor without adding a useless ASSERT. Complexity alone is not a finding.
...
- `.agents/skills/plan-simplicity-review/SKILL.md` and `.agents/skills/create-follow-up-plans/SKILL.md`
...
- Classifying each occurrence, determining worth, comparing simpler current mechanisms and the ASSERT/defer floor, tracing root cause versus bandaid, naming actual consumers and benefits, and checking representation fit/speculative machinery for the simplicity lens.
...
- Directive-to-implementation/consumer traceability, hidden defaults, missing mirrors, or orphan surfaces (CppPlanTraceAudit)
- Historical authorization, current documented subsystem/layer/client-server/CPU-GPU ownership boundaries, or placement/affinity review (CppScopeBoundaryAudit)
...
- Every applicable authority chain is read and every target appears exactly once in the simplicity shard coverage; every shard is at most 80,000 `bt-token-v1` or is split at a valid boundary.
- Every shard has one fresh Luna report and the consolidator's Temp index proves exact coverage with no missing, duplicate, or baseline-mismatched row; every report contains occurrence, confidence, cost comparison, actual consumer, simpler alternative, and severity evidence for each candidate.
