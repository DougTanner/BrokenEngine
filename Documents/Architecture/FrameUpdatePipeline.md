...
Each window is a set of nested scoped workbuffer reservations; their release order and the no-heap, no-growth rule that holds while a window is live are engine contracts ([`Engine/Source/Frame/AGENTS.md`](../../Engine/Source/Frame/AGENTS.md)). Under those rules acquisition batch rows and results come from fixed-size stack chunks in Missiles Update and from single-entry stack storage at player spawn. No context outlives its phase. The ownership layer is not a window and is never built from a phase; its by-value lifetime and post-tick main-thread-only restriction are engine contracts in that same document.
