...
- The log core formats into the thread's `ThreadLocal` log buffer, or a thread-local fallback line buffer on a thread with none — a foreign thread, a crash handler, or static initialization/destruction. Workbuffer-backed formatters have no fallback and ASSERT a live `ThreadLocal`. Emission is serialized; the crash snapshot ring does not wrap, while the ring that agent queries read — one shared ring buffer spanning all categories — does. Keep formatting allocation-free before entering either ring.
