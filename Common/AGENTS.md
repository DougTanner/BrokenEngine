...
- Payload structs whose raw object representation is written into a chunk declare every alignment gap as an explicit zero-initialized pad member. A compiler-inserted gap holds indeterminate bytes that copies carry into the emitted chunk, so exports from identical inputs produce differing bytes and differing content CRCs; the accompanying offset `static_assert` locks the layout the pad establishes.
...
Use `gpThreadLocal->mWorkbuffer` for transient buffers in tracked Engine/Game paths. `Push()` and `PushBuffer<T>()` return move-only RAII handles; keep all views inside the owning frame. Frames begin 16-byte aligned, so typed SIMD scratch is safe. Its storage is a `StableVector` (`StableVector.h`), so handles, pointers, spans, and views stay valid across a grow, while `DEBUG_BREAK()` and a `kError` log remain the under-sizing signal: size the buffer before the hot path instead of relying on growth.
...
- Use `ASSERT`, `CHECK_HRESULT`, and `VERIFY_SUCCESS`; standard `assert` is deliberately unavailable. `DEBUG_BREAK()` always logs one `kWarning` naming its call site, so a break stays visible with no debugger attached, and breaks into the debugger only when one is attached; the helpers behind the macros above throw after that break. `DEBUG_BREAK_NO_LOG()` is the non-logging variant reserved for a path that must not take the log mutex or allocate, such as crash handling (`../Engine/Source/AGENTS.md`). All three macros end in an `_Analysis_assume_` line that generates no code and exists only to tell the Release `/analyze` static-analysis build that the checked condition holds afterwards; without it the ordinary `ASSERT(p); p->...` pattern floods that build with null-dereference warnings, so keep it and give any new validation macro the same tail.
- Reuse Common's small utilities instead of writing local equivalents: `AlignedMemory.h` (`MakeAligned` / `AlignedUniquePtr` for 64-byte-aligned SIMD storage), `Smoothed.h` (`Smoothed<T, COUNT>` rolling average, `InTheLastSecond` rate counter), `Timer.h`, `ScopedLambda.h` (run a lambda when the scope exits), `StableVector.h` (`StableVector<T>`, grow-only storage that commits pages inside one address-space reservation taken on first use, so its base address never moves and growth past the reservation asserts), and the `StringUtils.h` / `FileUtils.h` helpers. `Common.h` is the full index.
