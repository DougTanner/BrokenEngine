...
Edit the Markdown view of a loaded Gaea terrain. Only the invoking session edits `Temp/<name>.md`; the only script this skill runs is the read-only validator in step 3 below. The companion `.passthrough.json` is read-only here; if a change requires altering ports or modifiers, document the limitation and ask the user.
