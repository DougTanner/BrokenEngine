...
- `broken-engine-compile-invoke-result/v1` — the build never ran. `status`/`code`/`message` name the block; exit `2` is a structured block (parameter contract, context, provisioning, missing AgentTools or target, data-mode authorization) and exit `1` is an internal error. Report the exact `code` and `message` and stop; never present it as a build result.
