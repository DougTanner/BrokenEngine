...
2. Only `status` `pass` (exit 0) is usable; `blocked` (exit 2) or `error` (exit 1) means the changed-file list is unavailable — report that outcome instead of reconstructing the list inline. Done when the run's status is `pass`, or the non-pass outcome is reported in place of a review.
3. The entries list is capped at 500 rows and the regions table at 400, and shader selection depends on both, so read `truncation.entries`, `truncation.regions`, and `truncated`: an emitted count below the full count in either one makes the selection incomplete, which blocks the review exactly as a non-pass status does. Done when both emitted counts match their full counts, or the incomplete selection is reported as blocking.
4. An untracked shader file appears only when the caller supplies it with `-IncludeUntracked <comma-separated paths>`, and `counts.unlistedUntracked` reports how many untracked files the run did not list. Include the transitive shader headers those files reach. Done when the review set holds the in-scope files plus every transitive shader header they reach.
5. Read the applicable shader `AGENTS.md`, each changed file, its nearby producers/consumers, and the relevant whole function. Done when each of those has been read.
6. Search `ShaderFunctions.h` and family `*Common.h` files before recommending new helper logic. Done when every helper this review would recommend has been checked against those files.
7. Trace every changed shader-facing header in both directions:
   ...
   Done when every changed shader-facing header has both directions traced.
8. Review changed dual-language declarations against the actual block qualifier and CPU representation. Done when every changed dual-language declaration has been checked against both.
9. Compare field order, scalar widths, array strides, offsets, descriptor constants, writes, and binding roles. Do not infer layout from a generic `vec3` rule when `layout(scalar)` applies. Done when each of those attributes has been compared.
10. Apply the correctness checks in the footgun reference. Done when every check in that reference has been applied to the changed regions.
11. Report only reachable failures supported by the changed code and repository evidence. Do not turn a generic checklist item into a finding. Done when every recorded finding cites changed code or repository evidence.
12. Emit an external-claim request per `/verify-external-claims` ([`../../verify-external-claims/SKILL.md`](../../verify-external-claims/SKILL.md), `## External Claim Requests`) covering one single checkable statement for every finding that depends on a non-obvious GLSL, Vulkan, extension, device, or compiler claim, using only the official source set in [`../../verify-external-claims/references/worker.md`](../../verify-external-claims/references/worker.md). Do not browse directly. A pending verdict makes the review `NEEDS_ACTION` and keeps the dependent finding unconfirmed. Done when every such finding carries one single-claim request and the recommendation reflects any pending verdict.
13. Keep locally provable repository-contract findings separate. Done when no locally provable repository-contract finding sits inside an external-claim request.
14. For a Tier-2+ change, also run the scope authorization, minimality, and KISS passes in [`../../../references/scope-authorization.md`](../../../references/scope-authorization.md) over the changed shader regions step 1 already produced. Done when each pass has a verdict for every changed shader region, or the change is Tier 1.
15. Report the result in the `Scope:` field. Done when the `Scope:` field carries that verdict.
16. Return the report. Done when the handoff carries every section the public contract's form requires.
17. A shader-facing shared header has both C++ and GLSL surfaces, so explicitly require `/repo-code-review` as the sibling domain review when such a header changed; this review does not replace it. Done when the report names `/repo-code-review` as required whenever a changed file is a shader-facing shared header.
18. Report a changed shader over ~5,000 `bt-token-v1` (measure every changed shader in one batched run: `pwsh -NoProfile -Command "& '.agents/scripts/Measure-Tokens.ps1' -Path 'a','b','c' -Json"` — use `-Command`, not `-File`, because under `-File` the comma-separated list binds as one filename and the run fails; the single-path `-File ... -Path <path>` form still works for one file) as a size observation in `Residuals`; splitting it via `ShaderFunctions.h`/`*Common.h` is follow-up work, not part of this findings-only review. Done when every changed shader over that size has a `Residuals` size observation.

## Rules

- Never edit the reviewed files. Delegated-review conduct: [`../../../references/subagent-reporting.md`](../../../references/subagent-reporting.md).
- Cover correctness and performance, not style. Treat source-adjacent rationale as required only when a non-obvious mathematical, numerical, coordinate, ordering, layout, or hardware assumption carries correctness or measured performance.
- Read the footgun reference ([`shader-footguns.md`](shader-footguns.md)) on every review. It holds the correctness checks this review applies: numerical domains, coordinates, interpolation and sampling, synchronization and data races, subgroup operations, shared CPU/GLSL layout, performance evidence, algorithm checks, and optional stages.
