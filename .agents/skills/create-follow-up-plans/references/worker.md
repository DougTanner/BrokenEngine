...
After editing an existing tracked Plan, run `pwsh -NoProfile -File .agents/scripts/Test-PlanSchedulerState.ps1`. If it reports `worktree-cli.missing`, stop and report that authorized primary maintenance through `/compile` is required. Require `status: valid` and `code: ok`; record its `notices` and `healedClaims`. Never inspect machine-local claims.
