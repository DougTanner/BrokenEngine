...
- Judge an `AGENTS.md` against the advisory code-scaled budget reported by [`Get-AffectedAgentsDocs.ps1`](../scripts/Get-AffectedAgentsDocs.ps1), which owns the calculation and its inputs.
...
- An over-budget verdict never justifies deleting an operative rule or contract. Remove inventories, repeated introductions, navigation pointers, duplicated mechanics, and redundant prose first; if needed guidance still leaves the document over budget, retain it and report the advisory excess.
