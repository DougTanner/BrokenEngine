...
- A *hub* is an AGENTS.md whose directory has descendant AGENTS.md documents below it; an AGENTS.md with no descendant AGENTS.md below it is a *leaf*.
...
- A size target never justifies deleting a sentence that still changes a decision. When a needed sentence pushes a file over its target, report the excess.
