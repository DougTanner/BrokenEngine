...
4. Run the save wrapper. One call resolves the input and output paths, checks Python, and runs the saver. Done when that one call has returned.
   ...
   - The bundled [`save_terrain.py`](../gaea2-shared/scripts/save_terrain.py) owns parsing, reconstruction, and the `$id`, `$ref`, and port-parent fixups; do not reproduce or simplify those operations.
