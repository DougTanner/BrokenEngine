...
- Engine formatters own shared CPU text and every overlay screen: CPU, GPU, and Network. The memory text on the CPU and GPU screens reads the grid state stored on `GameBase` and the Network screen reads client session state, both through `game::gpGame`, so `ProfileScreens.cpp` and `ProfileNetworkScreen.cpp` are deliberate engine consumers of the game header. The Network screen reuses the FPS-header slot, which is free because the FPS header renders only on the CPU and GPU screens. Server formatting must remain server-safe and allocation-disciplined.
