...
- Statistic aggregation runs per tick only when `kbProfiling`; when disabled, the entity totals stay zero and their rows paint that zero, the Profile tab's counter rows stay hidden, and the Profile tab still paints its own header and non-aggregated rows, while tick/time and the map stay live.
