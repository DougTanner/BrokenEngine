...
- Staging storage grows lazily, with no allocation-tracking suppression. The descriptor rebind follows GPU buffer recreation, not that CPU growth; steady-state frames update mapped data only, preserving the renderer's record-once command buffers.
