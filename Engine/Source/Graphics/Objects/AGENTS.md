...
- Pipeline creation trusts shader reflection only after validating the expected vertex input and descriptor shape. Pack-derived model ranges, material starts, and present material texture indices are file trust boundaries and throw `common::CorruptStreamException` when invalid; an absent material texture selector leaves its paired index unused.
