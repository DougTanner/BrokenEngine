...
- Debug builds route mimalloc output to VS Output; all builds hook `SIGABRT` into `engine::HandleException` so an `abort()` — including a terminate from a thread outside `MainThread`'s try/catch — still writes a crash report. Shutdown reports peak heap and trips `DEBUG_BREAK` if committed memory exceeded the reserve (arena undersized).
