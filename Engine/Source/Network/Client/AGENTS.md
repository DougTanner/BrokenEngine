...
- Static data requires matching coord identity on `kWaitingFullState`/`kSubscribing`, exact epoch on `kWaitingFullState`, and a newer retained epoch on `kSubscribing`/`kUnsubscribed`. A coord mismatch drops silently because full state owns ghost unsubscribe. Static data buffers without slot mutation; an island CRC with no loaded template is corrupt, not stale, because the handshake already matched the island manifest, so it takes the client-fatal corrupt-input response (`../AGENTS.md`). The payload carries no frame area; every consumer reads the one constant area ([Frame](../../Frame/AGENTS.md)).
...
- `ClientDesyncCore` owns debug-frame correlation and repeated-desync escalation to disconnect. Real desync freezes ACK tracking and, only when `kbDesyncDebugFrames` is enabled, clones the mismatching client frame for comparison and requests a debug frame; disabled builds do none of these. The game Agent owns synthetic full-state fixture state and policy; the core retains only its temporary stall and reset hooks, which are cleared on fixture completion, reset, and session destruction. The game-side seam is documented in the game client-session leaf.
- Client agent fixtures enabled by `kbDebugInput` that probe subscriptions must take their evidence from the normal receive, classification, drain, ACK, and reset paths. Keep synthetic send suppression scoped to the injected receive call, and bound deferred observation so a fixture cannot alter subscription policy. Engine fixture lifetime follows the [Agent lifecycle contract](../../Agent/AGENTS.md#architecture).
