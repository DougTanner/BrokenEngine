...
- Static data applies the same coord-identity check as full state on `kWaitingFullState`/`kSubscribing` slots (silent drop on coord mismatch — the full-state path owns the ghost unsubscribe), with the exact-match guard against the server-assigned epoch only on `kWaitingFullState`; `kSubscribing` and `kUnsubscribed` admit out-of-order static data (buffers only, no slot mutation) only past the strictly-newer retained-epoch guard above. Static data that passes those guards is dropped whole before queueing when any placement's island CRC has no loaded island template.
- The frame area a static-data payload carries is neither trusted nor rejected on mismatch: the receive path recomputes it from the coord the payload carries (`../../Frame/AGENTS.md`).
...
The CRC fast path, the past-frame ring with its rollback orchestration, the replay tick driver, and the per-coord dispatcher live here. The dispatcher builds the work list of eligible coords, reconciles them in parallel — each worker writing only its own coord's frames — and merges profiling counters and the first-in-slot-order unresolved CRC mismatch into one result. The game supplies the payloads they replay, the transfer materialization they call, and the policy deciding when to reconcile and how to answer a desync.

- Apply transfer `StatusChange`s after each replayed tick, matching the server's Destroy/Spawn order, and recompute the Frame CRC whenever transfers changed the frame. The server recomputes the same way after its own transfers land (`../Server/AGENTS.md`), so a replayed frame matches the broadcast CRC only if both sides recompute; dropping this side's recompute reports every cross-cell transfer as a desync.
- `ReconcileReplay.h` names game Frame, input, and status-change types, and `ClientDesyncCore.h` names the game Frame its captured snapshot holds — the same deliberate exception `ServerTransferManager` takes (`../Server/AGENTS.md`), because the payload being replayed or captured is game-defined while the rollback, replay, and desync machinery is not. Both therefore stay out of the `Engine.h` aggregation and their consumers include them directly.

## Other

- Desync debug mode: `ClientDesyncCore` owns the debug-frame request and correlation and the repeated-desync window that escalates to disconnect, calling back into the game client session for the game-side desync seam (`../../../../Projects/BrokenEngineSandbox/Source/Network/Client/AGENTS.md`). Real desync handling enters this mode only in game builds with `kbDesyncDebugFrames` enabled; it freezes the ACK floor (received-tick tracking becomes a no-op) so the server keeps resending while the captured debug frame is inspected. Disabled builds never request a debug frame or freeze ACK tracking for a real desync. The synthetic agent full-state fixture explicitly enters the same engine mode regardless of that compile flag and freezes ACK tracking until the fixture is cleared or reset.
...
- Game session + reconciliation policy: `../../../../Projects/BrokenEngineSandbox/Source/Network/Client/AGENTS.md`
