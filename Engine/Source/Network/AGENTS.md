...
- Load generation identifies the server's current debugging-load world across connection bootstrap and load-scoped subscription traffic. Both receivers compare it with their current generation before applying the slot epoch or mutating transport/game state; a mismatch logs a Network warning and returns. Epoch remains the reuse counter within one load generation. The wire and reset behavior is defined in [Network architecture](../../../Documents/Architecture/Network.md#debug-load-generation).
