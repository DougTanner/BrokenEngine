...
- Spawn assignment completes when the spawn is minted, on the advancing update that builds its status change: the player row is born owned by the requesting client's GUID carried in the spawn status change, and no pairing pass runs after the tick. That GUID is the persistent relink key.
- Fleet navigation defers flagship updates through `StatusChange`s. Within `BuildFrameInputs`, queued player updates, fleet timers and pending flagship updates, waiting-client spawn construction and assignment, the agent status-change drain, plus broadcast run only on advancing updates, staying deferred through paused and other zero-tick updates. Spawn construction precedes the agent status-change drain.
