...
- The engine runtime owns the active set and calls two hooks here: this layer contributes every coordinate holding at least one Player, and forwards ownership of each departing current Frame to the engine replay owner before the runtime erases the coordinate storage. The [File Replay contract](../../../../../Engine/Source/File/AGENTS.md#replay-streams) owns retention after that handoff.
- Replay uses coordinates with live readers and consumes the recorded FrameInput order exactly without sorting it again; generation activation follows the [File Replay contract](../../../../../Engine/Source/File/AGENTS.md#replay-streams), while live-input and transfer publication ordering is supplied by the engine-owned managers.
