...
- The tick phases implemented here are called by engine-owned `RunFrameTick` (`../../../../Engine/Source/Frame/AGENTS.md`). CRC-affecting fixed-tick decisions use `Frame`, `FrameInput`, `FrameStaticData`, or explicit phase parameters, not `game::gpGame`, wall-clock time, or client render state; if required input is unavailable, stop and resolve ownership.
...
- At save/replay and full-state read boundaries, normalize a non-finite spaceship-spawn timer to zero, reject a finite timer at or above the shared 0.5-second spawn interval as a corrupt stream, and leave smaller finite values unchanged before the loop that repeatedly subtracts the interval.
...
- `FrameInput` — the per-tick deterministic unit, a versioned serializable list of `StatusChange`s — is declared and serialized here (`FrameInput.h/.cpp`), compiles in both builds, and is consumed by the tick pipeline, server broadcast, and replay (`../../../../Engine/Source/File/AGENTS.md`). Instances are never built from hardware; the server broadcaster, client reconciler, and replay reader generate them. Bump `FrameInput::kiVersion` on any change to the stream format or to the `StatusChange` payload bytes a server build writes: replays validate the version, and size isn't checked for non-trivially-copyable types, so the bump is the only guard. It is also the only guard for the replay difference stream's post-dispatch channel layout, whose records carry `FrameInput` payloads, so a change there takes the same bump. A member that exists only under `BT_CLIENT` contributes no persisted byte, so its addition or removal takes no bump; a wire-visible change still takes the separate `engine::kuiProtocolVersion` bump owned by `../Network/AGENTS.md`. `Crc()` serves the replay `DifferenceStream` (skip writing identical consecutive differences) and diagnostics; reconciliation desync detection compares `Frame` CRCs, not `FrameInput::Crc()`.
...
- Missile and player homing acquire through the engine frame registry, whose context lifetime, allocation, and subscriber-count contracts are engine-owned (`../../../../Engine/Source/Frame/AGENTS.md`). This scope builds the only two query windows for it — the Missile-Update window in Missiles Update and the Player-Spawn window at player spawn — as specified in Frame Registry Query Windows (`../../../../Documents/Architecture/FrameUpdatePipeline.md`).
...
- `Frame::OwnershipLayer` maps the engine ownership layer onto the `PlayersPostRender` columns; its lifetime and write rules are engine contracts (`../../../../Engine/Source/Frame/AGENTS.md`).
