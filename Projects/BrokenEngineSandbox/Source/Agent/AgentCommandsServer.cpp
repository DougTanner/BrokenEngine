...
std::string_view ReplayCrcCapturePhaseName(GameSaveLoad::ReplayCrcCapturePhase ePhase)
{
	switch (ePhase)
	{
		case GameSaveLoad::ReplayCrcCapturePhase::kIdle: return "idle";
		case GameSaveLoad::ReplayCrcCapturePhase::kArmed: return "armed";
		case GameSaveLoad::ReplayCrcCapturePhase::kWriter: return "writer";
		case GameSaveLoad::ReplayCrcCapturePhase::kWriterComplete: return "writer-complete";
		case GameSaveLoad::ReplayCrcCapturePhase::kReader: return "reader";
		case GameSaveLoad::ReplayCrcCapturePhase::kReaderComplete: return "reader-complete";
	}
	DEBUG_BREAK();
	return "idle";
}

void WriteReplayCrcCaptureResult(const GameSaveLoad::ReplayCrcCaptureInfo& rCaptureInfo, int64_t iOffset, int64_t iLimit, nlohmann::json& rResult)
{
	rResult["phase"] = ReplayCrcCapturePhaseName(rCaptureInfo.ePhase);
	rResult["complete"] = rCaptureInfo.flags & GameSaveLoad::ReplayCrcCaptureFlags::kComplete;
	rResult["overflow"] = rCaptureInfo.flags & GameSaveLoad::ReplayCrcCaptureFlags::kOverflow;
	rResult["appendEnabled"] = rCaptureInfo.flags & GameSaveLoad::ReplayCrcCaptureFlags::kAppendEnabled;
	rResult["capacity"] = rCaptureInfo.iCapacity;
	rResult["total"] = rCaptureInfo.iSampleCount;
	rResult["offset"] = iOffset;
	rResult["limit"] = iLimit;

	nlohmann::json samples = nlohmann::json::array();
	const int64_t iEnd = std::min<int64_t>(rCaptureInfo.iSampleCount, iOffset + iLimit);
	for (int64_t i = iOffset; i < iEnd; ++i)
	{
		const GameSaveLoad::ReplayCrcCaptureSample& rSample = rCaptureInfo.samples.at(static_cast<size_t>(i));
		samples.push_back({
			{"tick", rSample.iTick},
			{"crc", rSample.uiCrc},
		});
	}
	rResult["samples"] = std::move(samples);
}

void CommandReplayCrcCapture(const nlohmann::json& rParams, nlohmann::json& rResult)
{
	if constexpr (!kbDebugInput)
	{
		throw std::runtime_error("replay_crc_capture requires kbDebugInput build");
	}
	else
	{
		if (!rParams.is_object())
		{
			throw std::runtime_error("replay_crc_capture requires string 'action'");
		}
		...
		if (action != "arm" && action != "read" && action != "clear")
		{
			throw std::runtime_error("'action' must be arm|read|clear");
		}
		for ([[maybe_unused]] const auto& [rKey, rValue] : rParams.items())
		{
			if (rKey == "action")
			{
				continue;
			}
			if (action == "read" && (rKey == "offset" || rKey == "limit"))
			{
				continue;
			}
			throw std::runtime_error("replay_crc_capture unknown parameter '" + rKey + "'");
		}

		int64_t iOffset = 0;
		int64_t iLimit = GameSaveLoad::kiReplayCrcReadPageLimit;
		if (action == "read")
		{
			const auto ParseNonnegativeInteger = [&rParams](const char* pcKey, int64_t iDefault)
			{
				if (!rParams.contains(pcKey))
				{
					return iDefault;
				}
				const nlohmann::json& rValue = rParams.at(pcKey);
				if (!rValue.is_number_integer() && !rValue.is_number_unsigned())
				{
					throw std::runtime_error(std::string("'") + pcKey + "' must be a nonnegative integer");
				}
				if (rValue.is_number_unsigned() && rValue.get<uint64_t>() > static_cast<uint64_t>(std::numeric_limits<int64_t>::max()))
				{
					throw std::runtime_error(std::string("'") + pcKey + "' is out of range");
				}
				const int64_t iValue = rValue.get<int64_t>();
				if (iValue < 0)
				{
					throw std::runtime_error(std::string("'") + pcKey + "' must be nonnegative");
				}
				return iValue;
			};
			iOffset = ParseNonnegativeInteger("offset", 0);
			iLimit = ParseNonnegativeInteger("limit", GameSaveLoad::kiReplayCrcReadPageLimit);
			if (iLimit < 1 || iLimit > GameSaveLoad::kiReplayCrcReadPageLimit)
			{
				throw std::runtime_error("'limit' must be in 1..256");
			}
		}

		const GameSaveLoad::ReplayCrcCaptureInfo& rCaptureInfo = gpGame->mGameSaveLoad.GetReplayCrcCaptureInfo();
		if (action == "arm")
		{
			if (gpGame->mActiveCoords.size() != 1)
			{
				throw std::runtime_error("replay_crc_capture arm requires exactly one active coordinate");
			}
			if (gpGame->mGameSaveLoad.IsRecording())
			{
				throw std::runtime_error("replay_crc_capture arm requires recording and replay to be inactive");
			}
			if (gpGame->mGameSaveLoad.IsReplaying())
			{
				throw std::runtime_error("replay_crc_capture arm requires recording and replay to be inactive");
			}
			if (gpGame->mGameFlags & engine::GameFlags::kSaveReplay)
			{
				throw std::runtime_error("replay_crc_capture arm requires recording and replay to be inactive");
			}
			if (gpGame->mGameFlags & engine::GameFlags::kLoadReplay)
			{
				throw std::runtime_error("replay_crc_capture arm requires recording and replay to be inactive");
			}
			gpGame->mGameSaveLoad.ArmReplayCrcCapture(gpGame->mActiveCoords.front());
		}
		else if (action == "clear")
		{
			if (gpGame->mGameSaveLoad.IsRecording())
			{
				throw std::runtime_error("replay_crc_capture clear requires recording and replay to be inactive");
			}
			if (gpGame->mGameSaveLoad.IsReplaying())
			{
				throw std::runtime_error("replay_crc_capture clear requires recording and replay to be inactive");
			...
			gpGame->mGameSaveLoad.ClearReplayCrcCapture();
		}
		else if (iOffset > rCaptureInfo.iSampleCount)
		{
			throw std::runtime_error("'offset' must not exceed capture total");
		}

		if (action != "read")
		{
			iOffset = 0;
			iLimit = GameSaveLoad::kiReplayCrcReadPageLimit;
		}
		WriteReplayCrcCaptureResult(rCaptureInfo, iOffset, iLimit, rResult);
	}
}
...
bool ExecuteAgentCommandServer(std::string_view cmd, const nlohmann::json& rParams, nlohmann::json& rResult)
{
	...
	if (cmd == "replay_crc_capture")
	{
		CommandReplayCrcCapture(rParams, rResult);
		return true;
	}
