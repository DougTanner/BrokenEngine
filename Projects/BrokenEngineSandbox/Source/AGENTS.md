...
- The game-owned persisted client settings — `TweaksSettings.bin` and `ClientState.bin` — are versioned POD written through the engine versioned-file helpers. Bump the owning version on layout change. Game, sound, and graphics settings persistence is engine-owned (`../../../Engine/Source/Ui/AGENTS.md`).
