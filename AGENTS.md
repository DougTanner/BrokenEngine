...
- Reporting work: separate built from verified, and name each required check still open — reserve "done" for after those checks close. State a defect with its evidence and effect; a verdict or count alone is not a finding. Compare options on the same criteria, evidence, detail, and tone — recommend one, but never describe the favorite by its benefit and the alternative by its risk.
